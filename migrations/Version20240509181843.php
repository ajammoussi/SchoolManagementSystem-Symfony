...
final class Version20240509181843 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql("-- Create trigger to update user table when admin table is updated
CREATE TRIGGER trg_admin_after_insert_update
AFTER INSERT OR UPDATE ON admin
FOR EACH ROW
BEGIN
    INSERT INTO user (email, roles, password)
    VALUES (NEW.email, 'ROLE_ADMIN', NEW.password)
    ON DUPLICATE KEY UPDATE email = NEW.email, roles = 'ROLE_ADMIN', password = NEW.password;
    END;");
$this->addSql("
-- Create trigger to update user table when student table is updated
CREATE TRIGGER trg_student_after_insert_update
AFTER INSERT OR UPDATE ON student
FOR EACH ROW
BEGIN
    INSERT INTO user (email, roles, password)
    VALUES (NEW.email, 'ROLE_STUDENT', NEW.password)
    ON DUPLICATE KEY UPDATE email = NEW.email, roles = 'ROLE_STUDENT', password = NEW.password;
END;");
$this->addSql("
-- Create trigger to update user table when teacher table is updated
CREATE TRIGGER trg_teacher_after_insert_update
AFTER INSERT OR UPDATE ON teacher
FOR EACH ROW
BEGIN
    INSERT INTO user (email, roles, password)
    VALUES (NEW.email, 'ROLE_TEACHER', NEW.password)
    ON DUPLICATE KEY UPDATE email = NEW.email, roles = 'ROLE_TEACHER', password = NEW.password;
END;");
    }
