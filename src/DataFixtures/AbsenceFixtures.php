...
class AbsenceFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager): void
    {
        try {
            foreach ($this->absences as $absence) {
                ...
                $student = $manager->find(\App\Entity\Student::class, $absence[0]);
                if ($student !== null) {
                    $absenceObj->setStudent($student);
                }
                // Fetch the Course entity using the ID
                $course = $manager->find(\App\Entity\Course::class, $absence[1]);
                if ($course !== null) {
                    $absenceObj->setCourse($course);
                }
                print_r(new \DateTime($this->students[0][7]));
                print_r(new \DateTime($absence[2]));
                $absenceObj->setAbsenceDate(new \DateTime($absence[2]));
